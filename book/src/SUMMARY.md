# Summary

[Introduction](introduction.md)
[Quick Start](quick-start.md)
[Libretro In Rust](libretro-in-rust.md)
[Hello World Core](hello-world-core.md)
[Running A Core](running-a-core.md)
[Frame Loop Basics](frame-loop-basics.md)
[Input](input.md)
[Audio](audio.md)
[OpenGL](opengl.md)

# API Reference

- [Core Lifecycle](api/core-lifecycle.md)
- [Content, AV, and Timing](api/content-av-timing.md)
- [Runtime Video and Audio](api/runtime-video-audio.md)
- [Input and Events](api/input-events.md)
- [Environment and Core Options](api/environment-options.md)
- [Memory, Savestates, and Serialization](api/memory-savestates.md)
- [Storage, Disks, and VFS](api/storage-disks-vfs.md)
- [Hardware Rendering and OpenGL](api/hardware-opengl.md)
- [Frontend Services](api/frontend-services.md)
- [Diagnostics and Performance](api/diagnostics-performance.md)
- [Raw ABI Boundaries](api/raw-abi.md)

# Example Cores

- [Hello World Core](examples/hello-core.md)
- [Software Core](examples/software-core.md)
- [Modern OpenGL Core](examples/demo-core.md)
- [Compatibility OpenGL Core](examples/retrocompat-core.md)

# Reference

- [Local Libretro Specs](reference/libretro-specs.md)
- [Rustdoc Map](reference/rustdoc-map.md)
- [Publishing and Validation](reference/publishing.md)
